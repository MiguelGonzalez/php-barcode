...
class Barcode {
    ...
    private $_encoder_class;
    private $number_checksum;
    ...
    function __construct($number = 123456789101, $scale = 2, $encoding = 'EAN13') {
        $this->number = $number;
        $this->scale = $scale;
        ...
        if(file_exists($encoding_class)) {
            ...
            $this->_encoder_class = new $encoding($number);

            $this->_encoder();
        } else {
            throw new Exception("Encoding class not found in " . $encoding_class);
        }
    }

    private function _encoder() {
        $this->_bars = $this->_encoder_class->get_bars($this->number);
        $this->number_checksum = $this->_encoder_class->get_number_ckecksum();
    }
    ...
    */
    ...
    protected function _drawBars() {
        $bar_color=ImageColorAllocate($this->_image, 0x11, 0x00, 0x00);
    ...
    */
    ...
    protected function _drawText() {
        ...
        $font = dirname(__FILE__) . '/lib/fonts/FreeSansBold.ttf';

        $fontsize = $this->scale*(7);
        $kerning = $fontsize*1;

        for($i=0;$i<strlen($this->number_checksum);$i++) {
            imagettftext($this->_image, $fontsize, 0, $x, $y, $text_color, $font, $this->number_checksum[$i]);
            if($i==0 || $i==6) {
                $x += $kerning*0.5;
            }
            $x += $kerning;
